...
Node-API native prebuilds for Linux x64 and Windows x64, plus the Linux
in-guest console helper. Unsupported platforms or forced native builds fall back
to `node-gyp`.
...
3. Attach both prebuild directories as GitHub Release tarballs.
4. Reassemble both directories in a clean Ubuntu publish job.
5. Run `NODE_VMM_PACK_REQUIRE_WIN32=1 node scripts/pack-check.mjs`.
6. Run `npm publish --ignore-scripts --access public`.
...
prebuild. Before a manual publish, restore both release prebuild directories
