...
[![Runtime](https://img.shields.io/badge/runtime-Linux%2FKVM%20%7C%20Windows%2FWHP-2ea44f.svg?style=flat)](#requirements)
...
Linux/KVM and Windows/WHP backends. It is built for sandbox-like workloads where
Node developers want process-shaped ergonomics with VM isolation: run code, boot
app servers, publish ports, pause/resume, and throw writable overlays away.
...
| Workflow | Linux/KVM | Windows/WHP | Extra host dependency |
| --- | --- | --- | --- |
| Import SDK / inspect `features()` | Yes | Yes | None |
| Build from OCI image | Yes | Yes, prebuilt first then WSL2 fallback | Linux: sudo/ext4 tools; Windows: WSL2 only on fallback |
| Build from Dockerfile or Git repo | Yes | Planned | Linux: sudo/ext4 tools |
| Run prepared rootfs with `--net none` | Yes | Yes | Linux needs `/dev/kvm`; Windows needs WHP |
| Run supported prebuilt image from cold cache | Yes | Yes, no WSL2 if release asset exists | Network access to GitHub Releases |
| Run `--image alpine:3.20` from cache | Yes | Yes | Cache must already contain the prepared rootfs |
| Run unsupported `--image ...` from cold cache | Yes | Yes, with WSL2 | OCI network access and ext4 builder |
| Interactive shell / getty console | Yes | Yes | Host terminal |
| `apk add`, DNS, outbound network | Yes | Yes | Linux TAP/NAT or Windows Slirp |
| TCP port forwarding | Yes | Yes | Linux TAP/NAT or Windows Slirp |
| Pause, resume, stop | Yes | Yes | Native backend |
| RNG device for guest entropy | Yes | Yes | Native backend |
| Multi-vCPU native runner | Yes | Yes | Full Linux guest SMP parity remains backend work |
| Persistent rootfs writes | Yes | Yes | Run an explicit `--rootfs` without `--sandbox` |
| Reset-on-exit rootfs writes | Yes | Yes | Use `--sandbox` / `--restore` or cached/prebuilt image runs |
| Persistent named root disks | Yes | Yes | `--persist` / SDK `persist` stores a root disk under `--cache-dir/disks`; `--reset` recreates it |
| Attached data disks | Yes | Yes | `--attach-disk` / SDK `attachDisks` maps existing disk files after `/dev/vda`, starting at `/dev/vdb` |
| Next.js, Vite React/Vue, Express, Fastify app servers | Yes | Smoke path in progress | Rootfs/image support for the app |
...
- virtio-mmio network through TAP/NAT on Linux and Slirp on Windows
- Docker-style TCP port forwarding on Linux/KVM and Windows/WHP
- WHP pause/resume/stop lifecycle controls
- WHP clocksource/timer path for Alpine guests
