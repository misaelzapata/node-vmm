...
import type { PortForward } from "./types.js";
...
export interface HvfFdtSmokeResult {
  backend: "hvf";
  dtbBytes: number;
  rootInterruptParent: number;
  rootDmaCoherent: boolean;
  gicPhandle: number;
  cpuCount: number;
  cpuEnableMethod: string;
  gicRedistributorBytes: number;
  timerInterrupts: number;
  timerIrqFlags: number;
  chosenRandomness: boolean;
  serial0Alias: string;
  stdoutPath: string;
  uartBase: number;
  uartSpi: number;
  uartClockPhandles: number;
  rtcBase: number;
  rtcSpi: number;
  rtcIntid: number;
  gpioBase: number;
  ...
  gpioIntid: number;
  gpioPhandle: number;
  gpioPowerKeyCode: number;
  fwCfgBase: number;
  fwCfgSize: number;
  fwCfgDmaCoherent: boolean;
  virtioBase: number;
  virtioStride: number;
  virtioCount: number;
  virtioDmaCoherent: boolean;
  virtioFirstSpi: number;
  virtioBlkBase: number;
  virtioBlkIntid: number;
  virtioNetBase: number;
  virtioNetIntid: number;
  pcieBase: number;
  pcieMmioSize: number;
  pciePioBase: number;
  pcieEcamBase: number;
  pcieEcamSize: number;
  pcieFirstSpi: number;
  pcieIntxCount: number;
  pcieInterruptMapEntries: number;
  pcieInterruptMapMaskDevfn: number;
  pcieInterruptMapMaskPin: number;
  pcieBusCount: number;
  pcieDmaCoherent: boolean;
  emptyTransportMagic: number;
  emptyTransportDeviceId: number;
}

export interface HvfPl011SmokeResult {
  backend: "hvf";
  console: string;
  ...
  frEmpty: number;
  frWithRx: number;
  risBeforeClear: number;
  misBeforeClear: number;
  risAfterClear: number;
  ibrd: number;
  fbrd: number;
  lcrH: number;
  cr: number;
...
export interface HvfDeviceSmokeResult {
  backend: "hvf";
  rtcNow: number;
  rtcLoaded: number;
  rtcPeripheralId0: number;
  rtcPrimeCellId0: number;
  gpioData: number;
  gpioDirection: number;
  gpioPeripheralId0: number;
  gpioPrimeCellId0: number;
  fwCfgSignature: string;
  fwCfgId: number;
  fwCfgCpus: number;
  fwCfgFileCount: number;
  emptyVirtioMagic: number;
  emptyVirtioDeviceId: number;
  emptyVirtioVendor: number;
  pcieEmptyVendor: number;
}
...
export type HvfRunConfig = NativeRunConfig;
...
export interface NativeHvfBackend {
  ...
  hvfFdtSmoke(): HvfFdtSmokeResult;
  hvfPl011Smoke(): HvfPl011SmokeResult;
  hvfDeviceSmoke(): HvfDeviceSmokeResult;
...
export const native: NativeBackend = {
  ...
  hvfFdtSmoke: () => requireNativeMethod("hvfFdtSmoke")(),
  hvfPl011Smoke: () => requireNativeMethod("hvfPl011Smoke")(),
  hvfDeviceSmoke: () => requireNativeMethod("hvfDeviceSmoke")(),
