...
napi_value RunVm(napi_env env, napi_callback_info info) {
  ...
  try {
    ...
    // the POSIX path in native/kvm/backend.cc:3074-3130 - same Uart::
    ...
    // shell that's already idle. Mirrors native/kvm/backend.cc:3074-3130.
    ...
    // Mirrors the IRQ0/PIT pattern at native/whp/backend.cc:4451: try the
